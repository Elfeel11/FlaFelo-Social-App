...
useEffect(()  => {
  ...
  return <>
  <div className=" grid gap-4 p-4 mx-auto max-w-2xl ">
    {posts.map((posts) => <div key={posts.id} className="bg-white w-full rounded-md shadow-md h-auto py-3 px-3 my-5">
  <div className="w-full h-16 items-center flex justify-between ">
    <div className="flex">
      <img className=" rounded-full w-10 h-10 mr-3" src={posts.user.photo} alt />
      <div>    
  ...
  <p>{posts.body}</p>
  {posts.image && <img src={posts.image} alt=""  className=" w-full object-cover h-100 mt-2 rounded-2xl" />}
